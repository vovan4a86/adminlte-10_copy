...
class SiteHelper {
	...
	public static array $weekdayRuByNum = [
		1 => 'Понедельник',
		2 => 'Вторник',
		3 => 'Среда',
		4 => 'Четверг',
...
//		self::generateSitemapIndex();
	...
	private static function generateFederalSitemap() {
        ...
        //Портфолио
        $items = Portfolio::wherePublished(1)->get();
        foreach ($items as $item) {
            $map->add_url($item->url);
        }
