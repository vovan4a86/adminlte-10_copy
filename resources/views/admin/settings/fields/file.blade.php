<div class="s-file-field">
	<label class="btn btn-success btn-sm">Загрузить файл
	...
	<div class="s-file-item">
		@if (isset($value) && $value || (!isset($value) && $setting->value))
			@if (array_search(strtolower(pathinfo(base_path().$setting::UPLOAD_PATH.($value ?? $setting->value), PATHINFO_EXTENSION)), \App\Models\Setting::$extensions) !== false)
				<span class="images_item">
					<img class="img-polaroid" src="{{ $setting::UPLOAD_URL.($value ?? $setting->value) }}" style="cursor:pointer;" onclick="popupImage($(this).attr('src'))">
					<a class="images_del" href="#" onclick="settingsFileDel(this, event)"><span class="glyphicon glyphicon-trash"></span></a>
				</span>
			@else
				<div class="margin">
					<a class="text-light-blue" target="_blank" href="{{ url($setting::UPLOAD_URL.($value ?? $setting->value)) }}">
